...
void driver_init_ADC(void)
{
	...
	if (do_calibration1) {
		...
		ESP_LOGI(TAG, "ADC%d Channel[%d] Cali Voltage: %d mV", ADC_UNIT_1 + 1, ADC1_CHAN1, voltage[0][1]);
	}
	vTaskDelay(pdMS_TO_TICKS(20));

	ESP_ERROR_CHECK(adc_oneshot_read(adc1_handle, ADC1_CHAN2, &adc_raw[0][2]));
	ESP_LOGI(TAG, "ADC%d Channel[%d] Raw Data: %d", ADC_UNIT_1 + 1, ADC1_CHAN2, adc_raw[0][2]);
	if (do_calibration1) {
		ESP_ERROR_CHECK(adc_cali_raw_to_voltage(adc1_cali_handle, adc_raw[0][2], &voltage[0][2]));
		ESP_LOGI(TAG, "ADC%d Channel[%d] Cali Voltage: %d mV", ADC_UNIT_1 + 1, ADC1_CHAN2, voltage[0][2]);
	}
	vTaskDelay(pdMS_TO_TICKS(20));

	ESP_ERROR_CHECK(adc_oneshot_read(adc1_handle, ADC1_CHAN3, &adc_raw[0][3]));
	ESP_LOGI(TAG, "ADC%d Channel[%d] Raw Data: %d", ADC_UNIT_1 + 1, ADC1_CHAN3, adc_raw[0][3]);
	if (do_calibration1) {
		ESP_ERROR_CHECK(adc_cali_raw_to_voltage(adc1_cali_handle, adc_raw[0][3], &voltage[0][3]));
		ESP_LOGI(TAG, "ADC%d Channel[%d] Cali Voltage: %d mV", ADC_UNIT_1 + 1, ADC1_CHAN3, voltage[0][3]);
	}
	vTaskDelay(pdMS_TO_TICKS(20));

	ESP_ERROR_CHECK(adc_oneshot_read(adc1_handle, ADC1_CHAN4, &adc_raw[0][4]));
	ESP_LOGI(TAG, "ADC%d Channel[%d] Raw Data: %d", ADC_UNIT_1 + 1, ADC1_CHAN4, adc_raw[0][4]);
	if (do_calibration1) {
		ESP_ERROR_CHECK(adc_cali_raw_to_voltage(adc1_cali_handle, adc_raw[0][4], &voltage[0][4]));
		ESP_LOGI(TAG, "ADC%d Channel[%d] Cali Voltage: %d mV", ADC_UNIT_1 + 1, ADC1_CHAN4, voltage[0][4]);
	}
	vTaskDelay(pdMS_TO_TICKS(20));

	ESP_ERROR_CHECK(adc_oneshot_read(adc1_handle, ADC1_CHAN5, &adc_raw[0][5]));
	ESP_LOGI(TAG, "ADC%d Channel[%d] Raw Data: %d", ADC_UNIT_1 + 1, ADC1_CHAN5, adc_raw[0][5]);
	if (do_calibration1) {
		ESP_ERROR_CHECK(adc_cali_raw_to_voltage(adc1_cali_handle, adc_raw[0][5], &voltage[0][5]));
		ESP_LOGI(TAG, "ADC%d Channel[%d] Cali Voltage: %d mV", ADC_UNIT_1 + 1, ADC1_CHAN5, voltage[0][5]);
	}
	vTaskDelay(pdMS_TO_TICKS(20));
}
